...
import ShowCarModal from "@/components/ShowCarModal.vue";
...
<template>
  <ShowCarModal :show="show" :data="data" @cancel="closeDetail()" />
  <DialogsWrapper />
  <div
        ...
        >
          ...
          <td class="px-6 py-4 text-right flex space-x-2 items-center">
            <button type="button" class="font-medium hover:underline">
              <span
                @click="enabledCar(car.id)"
                v-if="car.state != 1"
                class="text-indigo-600"
                >Activate</span
              >
              <span
                @click="disabledCar(car.id)"
                v-if="car.state == 1"
            ...
            <router-link
              :to="{ name: 'car.edit', params: { id: car.id } }"
